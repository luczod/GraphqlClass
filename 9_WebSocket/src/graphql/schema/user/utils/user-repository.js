...
const validateUserPassword = (userPassword) => {
  // letra minuscula, letra maiuscula, um numero e o tamanho
  //um por
  const passwordRegExp = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{6,30}$/;

  if (!userPassword.match(passwordRegExp)) {
    throw new UserInputError("Password must be follow rules.");
  }
};
